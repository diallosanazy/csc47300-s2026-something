...
export const initAddItem = () => {
  ...
  let imageDataURL   = null;   // base64 image string
  ...
  function handleFile(file) {
    ...
    reader.onload = function (e) {
      ...
      previewName.textContent = file.name || 'Preview';
      uploadZone.hidden = true;
      previewWrap.hidden = false;
      markDirty();
    };
  ...
  form.addEventListener('submit', function (e) {
    ...
    setTimeout(() => {
      if (editingItem) {
        window.location.href = toAppPath('item-detail.html?id=' + encodeURIComponent(item.id));
      } else {
        window.location.href = toAppPath('menu.html');
      }
    }, 1500);
  });
