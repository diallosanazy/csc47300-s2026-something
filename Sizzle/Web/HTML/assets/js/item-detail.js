// ── Item Detail – Frontend Logic ─────────────────────────
// Reads ?id= from URL, looks up item in hardcoded registry
// or sessionStorage, and populates the detail view.
...
(function () {
  ...
  el.modalConfirm.addEventListener('click', function () {
    ...
    if (isHardcoded(item.id)) {
      // Can't truly remove hardcoded items from HTML, so mark them
      // in sessionStorage as deleted so menu.js can hide them
      var deleted = [];
